  ...
  const fetchOrganizational = useCallback(async () => {
    try {
      ...
      const filteredData = data.filter(
        (board: any) => board.Type === "ฝ่ายสมาชิกสัมพันธ์และสวัสดิการ"
      );

      const processedData = filteredData.map((memberRelations: any) => ({
        id: memberRelations.Id,
        name: memberRelations.Name,
        position: memberRelations.Position,
        priority: memberRelations.Priority,
        type: memberRelations.Type,
        imagePath: memberRelations.ImagePath
          ? `${URLFile}${memberRelations.ImagePath}`
          : "",
      }));
